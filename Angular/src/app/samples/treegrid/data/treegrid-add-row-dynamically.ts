...
import { iuiTreeGridAddRowDynamicallyStyle } from './treegrid-add-row-dynamically.style';
...
export class TreeGridAddRowDynamically {
    ...
    addRoot(){
        ...
        this.treegrid.nativeElement.beginEdit(this.currentEditRow);
    ...
    // Cancels the editing process and closes the inline editor
    async cancelEdit(row: any){
        this.treegrid.nativeElement.endEdit();
    ...
    async saveRow(row: any, focusCell?: any){
        ...
        this.treegrid.nativeElement.endEdit(true);
        this.resetEditor();
        this.isNewRow = false;
    }
        ...
        switch (cell.cid){
            case 7: // Command Buttons
                if (row === this.currentEditRow)
                    return html`
                        <iui-button @mouseup="${() => this.saveRow(row)}" @touchstart="${() => this.saveRow(row)}" @keydown="${(e: any) => this.saveKeyDown(e, row)}">Save</iui-button>
                        <iui-button @mouseup="${() => this.cancelEdit(row)}" @touchstart="${() => this.cancelEdit(row)}" @keydown="${(e: any) => this.cancelKeyDown(e, row)}">Cancel</iui-button>
                    `
                else
                    return html`
                        <iui-button @mouseup="${() => this.editRow(row)}" @touchstart="${() => this.editRow(row)}" @keydown="${(e: any) => this.editKeyDown(e, row)}">Edit</iui-button>
                        <iui-button @mouseup="${() => this.removeRow(row)}" @touchstart="${() => this.removeRow(row)}" @keydown="${(e: any) => this.removeKeyDown(e, row)}">Remove</iui-button>
                    `
            // no default
        }
