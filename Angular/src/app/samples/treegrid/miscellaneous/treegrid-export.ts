...
@Component({
    selector: '',
    templateUrl: './treegrid-export.html',
    styleUrls: ['./treegrid-export.css']
})
export class TreeGridExport {
    @ViewChild('treegrid', { static: false }) treegrid!: ElementRef;

    public columns: Array<any> = [];
    public ctrlSize: any = { width: 725, height: 400 };
    ...
    exportJSON(){
        let data = this.treegrid.nativeElement.exportToJSON();
        console.log(data);
    }

    exportCSV(){
        let data = this.treegrid.nativeElement.exportToCSV();
