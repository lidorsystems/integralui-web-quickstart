...
@Component({
    selector: '',
    templateUrl: './grid-sorting-overview.html',
    styleUrls: ['./grid-sorting-overview.css']
})
export class GridSortingOverview {
    ...
    ngAfterViewInit(){
        ...
        this.grid.nativeElement.sort(this.sortColumn, this.sorting);
    ...
    onColumnClick(e: any){
        if (e.detail.column){
            ...
            if (e.detail.column.id === 4)
                this.grid.nativeElement.sort(e.detail.column, this.sorting, this.comparer);
            else
