...
import { iuiMenuCustomTemplateStyle } from './contextmenu-custom-template.style';

@Component({
    selector: '',
    templateUrl: './contextmenu-custom-template.html',
    styleUrls: ['./contextmenu-custom-template.css']
})
export class ContextMenuCustomTemplate {
    ...
    zoomIn(e: any, item: any){
        ...
        self.zoomInterval = setInterval(function(){
            ...
            self.contextmenu.nativeElement.refresh();
        ...
        if (item.text === 'Zoom')
            return html`
                <div class="contextmenu-cstpl-block">
                    ...
                    <div class="contextmenu-cstpl-zoom-block" @mouseup="${(e: any) => this.preventClosing(e)}">
                        <span class="contextmenu-cstpl-zoom-out" @mousedown="${(e: any) => this.zoomOut(e, item)}">-</span>
                        <span class="contextmenu-cstpl-zoom-value">${item.zoomValue}%</span>
                        <span class="contextmenu-cstpl-zoom-in" @mousedown="${(e: any) => this.zoomIn(e, item)}">+</span>
                    </div>
                </div>
            `;
