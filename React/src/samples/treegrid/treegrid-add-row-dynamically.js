...
class TreeGridAddRowDynamically extends Component {
    ...
    addChild(){
        ...
        this.insertRowInGrid(this.gridRef.current.ctrlRef.current.selectedRow);
        this.setFocusToCell();
    }
    ...
    insertRowInGrid(selRow){
        let row = this.createNewRow();
    
        this.currentEditRow = row;

        this.gridRef.current.insertRowAt(row, 0, selRow);
    ...
    moveRowToEnd(row){
        if (row){
            ...
            let parentRow = this.gridRef.current.getRowParent(row);
            if (parentRow)
                list = parentRow.rows;
    ...
    render() {
        return (
            <div>
                <h2>TreeGrid / Add Rows Dynamically</h2>
                <div className="sample-block" id="treegrid-add-row-dynamically">
                    <IntegralUIButtonComponent allowAnimation={this.state.isAnimationAllowed} size={this.state.buttonSize} theme={this.state.currentTheme} onClick={() => this.addRoot()}>Add Root</IntegralUIButtonComponent>
                    <IntegralUIButtonComponent allowAnimation={this.state.isAnimationAllowed} size={this.state.buttonSize} theme={this.state.currentTheme} onClick={() => this.addChild()}>Add Child</IntegralUIButtonComponent><br/>
                    <IntegralUITreeGridComponent ref={this.gridRef}
                        allowAnimation={this.state.isAnimationAllowed} 
                        allowFocus={true}
                        columns={this.state.columns} 
                        customStyle={iuiTreeGridAddRowDynamicallyStyle}
                    ...
                    <div className="feature-help">
                        ...
                        <p><span className="initial-space"></span>To add a new row use the button above the treegrid. Whenever the add button is clicked, a new row is created at first position in the treegrid and an inline cell editor appears. The cells in the last column contain action buttons that confirm or cancel the change of cell values.</p>
...
export default TreeGridAddRowDynamically;
